  ...
  void drawText(Canvas canvas, CandleEntity data, double x) {
    ...
    if (state == MainState.MA) {
      ...
      span = TextSpan(
        children: [
          if (data.up != 0)
            TextSpan(text: "BOLL:${format(data.mb)}    ", style: getTextStyle(this.chartColors.ma5Color)),
          if (data.mb != 0)
            TextSpan(text: "UB:${format(data.up)}    ", style: getTextStyle(this.chartColors.ma10Color)),
          if (data.dn != 0)
            TextSpan(text: "LB:${format(data.dn)}    ", style: getTextStyle(this.chartColors.ma30Color)),
        ],
      );
    }
    if (span == null) return;
    ...
    if (isLine == Bar.line) {
      ...
      switch (isLine) {
        case Bar.candle:
          ...
          break;
        case Bar.ohlc:
          drawOHLC(curPoint, canvas, curX);
          break;
        case Bar.hilo:
          drawHiLo(curPoint, canvas, curX);
          break;
        default:
          drawCandle(curPoint, canvas, curX);
          break;
...
//    }
  ...
  void drawMaLine(CandleEntity lastPoint, CandleEntity curPoint, Canvas canvas, double lastX, double curX) {
    for (int i = 0; i < (curPoint.maValueList?.length ?? 0); i++) {
      ...
      if (lastPoint.maValueList?[i] != 0) {
        drawLine(
            lastPoint.maValueList?[i], curPoint.maValueList?[i], canvas, lastX, curX, this.chartColors.getMAColor(i));
      }
    }
  }
  ...
  void drawOHLC(CandleEntity curPoint, Canvas canvas, double curX) {
    var high = getY(curPoint.high);
    var low = getY(curPoint.low);
    var open = getY(curPoint.open);
    var close = getY(curPoint.close);
    ...
    if (open >= close) {
      ...
      chartPaint.color = this.chartColors.upColor;
//      canvas.drawRect(Rect.fromLTRB(curX - r, close, curX + r, open), chartPaint);
      canvas.drawRect(Rect.fromLTRB(curX - r, open - 1, curX, open + 1), chartPaint);
      canvas.drawRect(Rect.fromLTRB(curX, close - 1, curX + r, close + 1), chartPaint);
      canvas.drawRect(Rect.fromLTRB(curX - lineR, high, curX + lineR, low), chartPaint);
    } else if (close > open) {
      // 实体高度>= CandleLineWidth
      if (close - open < mCandleLineWidth) {
...
//      canvas.drawRect(Rect.fromLTRB(curX - r, open, curX + r, close), chartPaint);
  ...
  void drawHiLo(CandleEntity curPoint, Canvas canvas, double curX) {
    var high = getY(curPoint.high);
    var low = getY(curPoint.low);
    var open = getY(curPoint.open);
    var close = getY(curPoint.close);
