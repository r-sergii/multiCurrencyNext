  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: CustomPaint(
        ...
        painter: DepthChartPainter(widget.bids, widget.asks, pressOffset, isLongPress, widget.fixedLength,
            widget.buyPathColor, widget.sellPathColor, widget.chartColors),
      ),
    );
  }
}
...
class DepthChartPainter extends CustomPainter {
  ...
  DepthChartPainter(this.mBuyData, this.mSellData, this.pressOffset, this.isLongPress, this.fixedLength,
      this.mBuyPathColor, this.mSellPathColor, this.chartColors) {
    mBuyLinePaint ??= Paint()
      ..isAntiAlias = true
  ...
  void drawSell(Canvas canvas) {
    ...
    for (int i = 0; i < mSellData!.length; i++) {
      ...
      if (i >= 1) {
        canvas.drawLine(Offset((mSellPointWidth! * (i - 1)) + mDrawWidth, getY(mSellData![i - 1].vol)), Offset(x, y),
            mSellLinePaint!);
      }
  ...
  void drawText(Canvas canvas) {
    ...
    rightHalfTP.layout();
    rightHalfTP.paint(
        canvas, Offset((mDrawWidth + mWidth - rightHalfTP.width) / 2, getBottomTextY(rightHalfTP.height)));
  ...
  void drawSelectView(Canvas canvas, int index, bool isLeft) {
    ...
    Rect rightRect =
        Rect.fromLTRB(mWidth - amountTP.width - 6, rightRectTop - 3, mWidth, rightRectTop + amountTP.height + 3);
  ...
  getTextPainter(String text, [Color color = Colors.white]) => TextPainter(
      text: TextSpan(text: "$text", style: TextStyle(color: color, fontSize: 10)), textDirection: TextDirection.ltr);
